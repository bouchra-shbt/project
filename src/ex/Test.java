...
public class Test extends GridSim
{
     ...
     *                      initialized GridSim.
     */
    public Test(String name, String destName, Link link,double A) throws Exception
    {
        super(name, link);
    ...
    public void body()
    {
        ...
        // if an event denotes end of simulation
            ...
            {
                System.out.println();
                write(super.get_name() + "maitre: exiting ...");
                break;
            }
        // if an event denotes another event type
            ...
            {      
                System.out.println();
                write("maitre: receive " +ev.get_data() + ", at time = " + GridSim.clock());
                // sends back an ack
                IO_data data = new IO_data(ev.get_data(), packetSize, destID_);
                write( "maitre Sending back " +R + ", at time = " + GridSim.clock() );
                // sends through Output buffer of this entity
                super.send(super.output, GridSimTags.SCHEDULE_NOW,NetUser.SEND_MSG, data);
            }
            else if (ev.get_tag() == NetUser2.SEND_MSG)
            {
                System.out.println();
                write("maitre: receive " +ev.get_data() + ", at time = " + GridSim.clock());
                // sends back an ack
                IO_data data2 = new IO_data(ev.get_data(), packetSize, destID_);
                write( "maitre Sending back " +ev.get_data() + ", at time = " + GridSim.clock() );
                // sends through Output buffer of this entity
                super.send(super.output, GridSimTags.SCHEDULE_NOW,NetUser2.SEND_MSG, data2);
            }
            else if (ev.get_tag() == NetUser3.SEND_MSG)
            {   
                System.out.println();
                write("maitre: receive " +ev.get_data() + ", at time = " + GridSim.clock());
                // sends back an ack
                IO_data data3 = new IO_data(ev.get_data(), packetSize, destID_);
                write( "maitre Sending back " +ev.get_data() + ", at time = " + GridSim.clock() );
                // sends through Output buffer of this entity
                super.send(super.output, GridSimTags.SCHEDULE_NOW,NetUser3.SEND_MSG, data3);
            }
...
super.send(super.output, GridSimTags.SCHEDULE_NOW,GridSimTags.INFOPKT_RETURN,new IO_data(pkt,pkt.getSize(),pkt.getSrcID()));
    }
    /**
